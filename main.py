...
def validate_common_params(lang: str = "", country: str = "", max_articles: int = 10, page: int = 1) -> None:
    """Validate common parameters used by both search and headlines functions"""
...
def build_params(**kwargs) -> Dict[str, Any]:
    """Build request parameters, excluding None/empty values"""
...
async def search_news(
    ...
    lang: str = Field(default="", description=f"Language code (2 letters). Supported: {', '.join(SUPPORTED_LANGUAGES.keys())}"),
    country: str = Field(default="", description=f"Country code (2 letters). Supported: {', '.join(SUPPORTED_COUNTRIES.keys())}"),
    max_articles: int = Field(default=10, description="Number of articles to return (1-100)"),
    search_in: str = Field(default="", description="Search in specific fields: title, description, content (comma-separated)"),
    nullable: str = Field(default="", description="Allow null values for: description, content, image (comma-separated)"),
    date_from: str = Field(default="", description="Filter articles from this date (ISO 8601 format: YYYY-MM-DDTHH:MM:SS.sssZ)"),
    date_to: str = Field(default="", description="Filter articles until this date (ISO 8601 format: YYYY-MM-DDTHH:MM:SS.sssZ)"),
    sortby: str = Field(default="publishedAt", description="Sort by publication date or relevance"),
    ...
    # Validate parameters
    validate_common_params(lang, country, max_articles, page)
...
async def get_top_headlines(
    ...
    date_from: str = Field(default="", description="Filter articles from this date (ISO 8601 format: YYYY-MM-DDTHH:MM:SS.sssZ)"),
    date_to: str = Field(default="", description="Filter articles until this date (ISO 8601 format: YYYY-MM-DDTHH:MM:SS.sssZ)"),
    q: str = Field(default="", description="Additional search keywords to filter headlines"),
    page: int = Field(default=1, description="Page number for pagination")
) -> dict:
    """
